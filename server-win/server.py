...
LOG_TRANSFER_INTERVAL_SECONDS = 10
LOG_TRANSFER_ROOM_DELAY_SECONDS = 0.5
LOG_TRANSFER_DELETE_DELAY_SECONDS = 0.1
INITIAL_MAP_ROOM_DELAY_SECONDS = 0.5
STAFF_PIN_SYNC_INTERVAL_SECONDS = 3600
STAFF_PIN_SYNC_START_DELAY_SECONDS = 30

# Background cache (osvježavanje statusa soba u pozadini)
BG_CACHE_INTERVAL_SECONDS = 15          # Koliko često se osvježava keš (min 15s)
BG_CACHE_ROOM_DELAY_SECONDS = 0.5       # Pauza između soba (daje ESP32 prostora za live komande)
BG_CACHE_STALE_SECONDS = 30             # Koliko sekundi keš važi kao "svjež"
BG_CACHE_START_DELAY_SECONDS = 15       # Čekaj initial_map prije prvog ciklusa

# Session trajanja (JWT)
SESSION_GUEST_HOURS = 24
SESSION_ADMIN_HOURS = 8
SESSION_MANAGER_HOURS = 8
SESSION_MANAGER_ROOM_MINUTES = 30

# Server
SERVER_THREADS = 10

# Alexa/IFTTT termostat limiti
ALEXA_MIN_TEMP = 18
ALEXA_MAX_TEMP = 30
...
# Resolving registry — sprječava višestruke istovremene mDNS pretrage za isti uređaj
_resolving_in_progress = set()
_resolving_registry_lock = threading.Lock()

# Cache zadnjeg uspješno pročitanog statusa sobe.
# Ako live fetch faili (mrežni tresak), vraća se ovo umjesto defaulta
# da se spriječi lažno treperenje statusa (zauzeta → slobodna → zauzeta).
_room_status_cache = {}  # {soba_id: room_info_dict}
_room_status_cache_lock = threading.Lock()

# Background cache — pozadinski thread puni, endpointi čitaju
_bg_room_cache = {}       # {soba_id: { currentTemp, setpoint, isThermostatOn, lights, card_inserted, ... }}
_bg_room_cache_lock = threading.Lock()
...
def apply_config_settings():
    ...
    global LOG_TRANSFER_INTERVAL_SECONDS, LOG_TRANSFER_ROOM_DELAY_SECONDS, LOG_TRANSFER_DELETE_DELAY_SECONDS
    global INITIAL_MAP_ROOM_DELAY_SECONDS
    global STAFF_PIN_SYNC_INTERVAL_SECONDS, STAFF_PIN_SYNC_START_DELAY_SECONDS
    global BG_CACHE_INTERVAL_SECONDS, BG_CACHE_ROOM_DELAY_SECONDS, BG_CACHE_STALE_SECONDS, BG_CACHE_START_DELAY_SECONDS
    global SESSION_GUEST_HOURS, SESSION_ADMIN_HOURS, SESSION_MANAGER_HOURS, SESSION_MANAGER_ROOM_MINUTES
    global SERVER_THREADS
    global ALEXA_MIN_TEMP, ALEXA_MAX_TEMP
    ...
    LOG_TRANSFER_INTERVAL_SECONDS = _as_number(
        background_cfg.get('log_transfer_interval_seconds', LOG_TRANSFER_INTERVAL_SECONDS),
        LOG_TRANSFER_INTERVAL_SECONDS
    )
    LOG_TRANSFER_ROOM_DELAY_SECONDS = _as_number(
        background_cfg.get('log_transfer_room_delay_seconds', LOG_TRANSFER_ROOM_DELAY_SECONDS),
        LOG_TRANSFER_ROOM_DELAY_SECONDS
    )
    LOG_TRANSFER_DELETE_DELAY_SECONDS = _as_number(
        background_cfg.get('log_transfer_delete_delay_seconds', LOG_TRANSFER_DELETE_DELAY_SECONDS),
        LOG_TRANSFER_DELETE_DELAY_SECONDS
    )
    INITIAL_MAP_ROOM_DELAY_SECONDS = _as_number(
        background_cfg.get('initial_map_room_delay_seconds', INITIAL_MAP_ROOM_DELAY_SECONDS),
        INITIAL_MAP_ROOM_DELAY_SECONDS
    )
    BG_CACHE_INTERVAL_SECONDS = _as_number(
        background_cfg.get('bg_cache_interval_seconds', BG_CACHE_INTERVAL_SECONDS),
        BG_CACHE_INTERVAL_SECONDS
    )
    BG_CACHE_ROOM_DELAY_SECONDS = _as_number(
        background_cfg.get('bg_cache_room_delay_seconds', BG_CACHE_ROOM_DELAY_SECONDS),
        BG_CACHE_ROOM_DELAY_SECONDS
    )
    BG_CACHE_STALE_SECONDS = _as_number(
        background_cfg.get('bg_cache_stale_seconds', BG_CACHE_STALE_SECONDS),
        BG_CACHE_STALE_SECONDS
    )
    BG_CACHE_START_DELAY_SECONDS = _as_number(
        background_cfg.get('bg_cache_start_delay_seconds', BG_CACHE_START_DELAY_SECONDS),
        BG_CACHE_START_DELAY_SECONDS
    )

    security_cfg = cfg.get('security', {})
    SESSION_GUEST_HOURS = _as_number(security_cfg.get('session_guest_hours', SESSION_GUEST_HOURS), SESSION_GUEST_HOURS)
    SESSION_ADMIN_HOURS = _as_number(security_cfg.get('session_admin_hours', SESSION_ADMIN_HOURS), SESSION_ADMIN_HOURS)
    SESSION_MANAGER_HOURS = _as_number(security_cfg.get('session_manager_hours', SESSION_MANAGER_HOURS), SESSION_MANAGER_HOURS)
    SESSION_MANAGER_ROOM_MINUTES = _as_number(security_cfg.get('session_manager_room_minutes', SESSION_MANAGER_ROOM_MINUTES), SESSION_MANAGER_ROOM_MINUTES)

    SERVER_THREADS = int(_as_number(_get_nested(cfg, ['server', 'threads'], SERVER_THREADS), SERVER_THREADS))

    alexa_cfg = cfg.get('alexa_termostat_limiti', {})
    ALEXA_MIN_TEMP = int(_as_number(alexa_cfg.get('min_temp', ALEXA_MIN_TEMP), ALEXA_MIN_TEMP))
    ALEXA_MAX_TEMP = int(_as_number(alexa_cfg.get('max_temp', ALEXA_MAX_TEMP), ALEXA_MAX_TEMP))

def load_config():
    global CONFIG, APP_ENDPOINT_URL
    try:
        with open('config.json', 'r', encoding='utf-8') as f:
            ...
            for soba_id, soba_data in original_config.get('sobe', {}).items():
                ...
                soba_data['_fail_count'] = 0
            
            with config_lock:
                CONFIG = original_config
            apply_config_settings()

            # Vrati APP_ENDPOINT_URL iz prethodne sesije (persistiran pri register_app)
            saved_url = CONFIG.get('app_endpoint_url')
            if saved_url:
                APP_ENDPOINT_URL = saved_url
                logging.info(f"APP endpoint obnovljen iz config: {APP_ENDPOINT_URL}")
...
def save_config():
    ...
    with config_lock:
        try:
            ...
            # Remove runtime fields from all rooms before saving
            for soba_data in config_to_save.get('sobe', {}).values():
                soba_data.pop('cached_ip', None)
                soba_data.pop('_fail_count', None)
        ...
        try:
            ...
            if data.get('status') == 'success' and 'data' in data and 'ip' in data['data']:
                ...
                with config_lock:
                    ...
                    CONFIG['sobe'][soba_id]['_fail_count'] = 0  # Reset fail counter na uspješan resolve
                return ip_address
            else:
...
def safe_start_resolve(soba_id):
    """Pokreće resolve thread samo ako već nije u toku za tu sobu."""
    with _resolving_registry_lock:
        if soba_id in _resolving_in_progress:
            logging.debug(f"RESOLVE SKIP: {soba_id} se već traži, preskačem duplikat.")
            return
        _resolving_in_progress.add(soba_id)
    ...
    def _resolve_and_cleanup():
        try:
            ...
            with _resolving_registry_lock:
                _resolving_in_progress.discard(soba_id)

    threading.Thread(target=_resolve_and_cleanup, daemon=True).start()
...
def send_esp_command(soba_id, params, timeout=TIMEOUT_ESP_DEFAULT):
    ...
    try:
        ...
        with config_lock:
            if soba_id in CONFIG['sobe']:
                fail_count = CONFIG['sobe'][soba_id].get('_fail_count', 0) + 1
                CONFIG['sobe'][soba_id]['_fail_count'] = fail_count
                if fail_count >= 3:
                    CONFIG['sobe'][soba_id]['cached_ip'] = None
                    CONFIG['sobe'][soba_id]['_fail_count'] = 0
                    logging.warning(f"SOFT-FAIL: {soba_id} dostigao {fail_count} greške zaredom — brišem cached IP")
                else:
                    logging.warning(f"SOFT-FAIL: {soba_id} greška {fail_count}/3 — zadržavam stari IP")
...
def compress_response(response):
    ...
    # Kompresuj samo tekstualne sadržaje (HTML, JSON, CSS, JS)
        ...
        try:
            ...
            if soba_id:
                with config_lock:
                    ...
                    has_alexa = soba_cfg.get('has_alexa', False)
                    has_rampa = 'rampa' in soba_cfg
        except Exception:
            pass
    return render_template('soba.html', has_alexa=has_alexa, has_rampa=has_rampa)
            ...
            for soba_id, soba_data in CONFIG.get('sobe', {}).items():
                if soba_data.get('guest_pin') == pin:
                    token_payload = {
                        ...
                        'port': soba_data['port'],
                        'tip': 'gost',
                        'exp': datetime.utcnow() + timedelta(hours=SESSION_GUEST_HOURS)
                    }
...
def api_control():
    ...
    try:
        ...
        with config_lock:
            ...
            esp_target_soba_id = soba_id  # Default; rampa može preusmjeriti na drugi ESP

            # Specijalna obrada za thermostat_power, open_door i rampa
            if uredjaj == 'thermostat_power':
                ...
                rampa_cfg = soba_config.get('rampa')
                if not rampa_cfg:
                    return jsonify({'success': False, 'message': 'Rampa nije definirana za ovu sobu'}), 400
                esp_target_soba_id = rampa_cfg.get('soba_id', soba_id)
                params = {
                    'CMD': 'ESP_PULSE_PIN',
                    'PIN': str(rampa_cfg.get('pin')),
                    'PAUSE': str(rampa_cfg.get('pause', 10))
                }
            else:
        ...
        logging.info(f"GOST KONTROLA ({soba_id}): Uređaj '{uredjaj}' -> {params} (target ESP: {esp_target_soba_id})")
        response, message, json_resp = send_esp_command(esp_target_soba_id, params)
        
        logging.info(f"[DEBUG] Odgovor send_esp_command: response={'OK' if response else 'None'}, message='{message}'")
        
        if response and response.ok:
            # SAFEGUARD 1: Invalidiraj background cache — sljedeći poll padne na live fetch
            with _bg_room_cache_lock:
                _bg_room_cache.pop(soba_id, None)
...
def api_admin_login():
    ...
    if password == _get_nested(CONFIG, ['security', 'admin_password'], CONFIG.get('admin_password')):
        token_payload = { 'tip': 'admin', 'exp': datetime.utcnow() + timedelta(hours=SESSION_ADMIN_HOURS) }
        token = jwt.encode(token_payload, app.config['SECRET_KEY'], algorithm='HS256')
        response = make_response(jsonify({'success': True}))
...
def api_manager_login():
    ...
    if pin == manager_pin:
        token_payload = { 'tip': 'manager', 'exp': datetime.utcnow() + timedelta(hours=SESSION_MANAGER_HOURS) }
        token = jwt.encode(token_payload, app.config['SECRET_KEY'], algorithm='HS256')
        response = make_response(jsonify({'success': True}))
...
def manager_soba_redirect(soba_id):
    ...
    with config_lock:
        ...
        token_payload = {
            ...
            'exp': datetime.utcnow() + timedelta(minutes=SESSION_MANAGER_ROOM_MINUTES) # Kratkotrajni token
        }
        token = jwt.encode(token_payload, app.config['SECRET_KEY'], algorithm='HS256')
    ...
    try:
        ...
        if komanda == 'SET_PIN': 
            ...
            try:
                ...
                if final_value < ALEXA_MIN_TEMP:
                    final_value = ALEXA_MIN_TEMP
                    logging.info(f"IFTTT/ALEXA: Vrijednost {temp_trazena} je ISPOD minimuma. Postavljam na {final_value}°C")
                elif final_value > ALEXA_MAX_TEMP:
                    final_value = ALEXA_MAX_TEMP
                ...
                params['VALUE'] = str(ALEXA_MIN_TEMP) # Vrati na minimum ako je greška
        else: 
...
def api_get_guest_status():
    ...
    try:
        ...
        # === BACKGROUND CACHE: pokušaj pročitati iz keša ===
        with _bg_room_cache_lock:
            cached = _bg_room_cache.get(soba_id)

        if cached and (time.time() - cached.get('last_update', 0)) < BG_CACHE_STALE_SECONDS:
            # Keš svjež — vrati odmah bez kontaktiranja ESP32
            final_status = {}
            if cached.get('currentTemp') is not None:
                final_status['currentTemp'] = cached['currentTemp']
                final_status['setpoint'] = cached.get('setpoint', 22.0)
                final_status['isThermostatOn'] = cached.get('isThermostatOn', False)
            if cached.get('lights'):
                final_status['lights'] = cached['lights']
            logging.info(f"GOST STATUS ({soba_id}): Iz BG_CACHE (age={time.time() - cached['last_update']:.0f}s)")
            return jsonify({'success': True, 'status': final_status})

        # === FALLBACK: Keš prazan ili zastareo — live fetch (isto kao prije) ===
        current_temp, setpoint = 22.0, 22.0
        is_thermostat_on = False
        thermostat_fetched = False  # Da li je GET_ROOM_TEMP uspio
        light_states = {}
        ...
        final_status = {}
        # SAFEGUARD 3: šalji termostat/lights SAMO ako smo uspješno dobili podatke.
        # Ako GET_ROOM_TEMP timeout-uje, ne šaljemo termostat stanje → frontend drži zadnje.
        if thermostat_fetched:
            final_status['currentTemp'] = current_temp
            final_status['setpoint'] = setpoint
            final_status['isThermostatOn'] = is_thermostat_on
        # Šalji lights SAMO ako smo uspješno dobili podatke sa GET_PINS.
        # Prazan dict {} znači "nijedan GET_PINS nije uspio" — NE znači "sva svjetla ugašena".
        # Ako ne pošaljemo lights, frontend drži zadnje poznato stanje (ne resetuje na OFF).
        if light_states:
            final_status['lights'] = light_states
...
# ----------------------------------------------------------------------
#  POZADINSKI PROCES ZA AŽURIRANJE IP ADRESA + BACKGROUND CACHE
# ----------------------------------------------------------------------
def _refresh_room_cache(soba_id):
    """Osvježava background cache za jednu sobu. Poziva se iz background_resolver_task."""
    # Ne bombarduj ESP32 komandama dok je firmware update aktivan!
    if is_update_active(soba_id):
        logging.info(f"BG_CACHE: Preskačem {soba_id} — firmware update u toku.")
        return

    with config_lock:
        cfg = CONFIG['sobe'].get(soba_id, {})
        if not cfg.get('cached_ip'):
            return
        uredjaji = cfg.get('uredjaji', {})
        termostat_id = uredjaji.get('termostat_set', {}).get('ID')
        pin_ctrl_id = uredjaji.get('pin_controller', {}).get('ID')
        scene_ctrl_id = uredjaji.get('scene_controller', {}).get('ID', pin_ctrl_id)
        controller_to_pins = {}
        for dev_name, dev_data in uredjaji.items():
            if dev_name.startswith('light_') and 'ID' in dev_data and 'PIN' in dev_data:
                ctrl_id = dev_data['ID']
                pin_num = dev_data['PIN']
                if ctrl_id not in controller_to_pins:
                    controller_to_pins[ctrl_id] = {}
                controller_to_pins[ctrl_id][pin_num] = dev_name
    ...
    # 1. GET_ROOM_TEMP
    if termostat_id:
        _, _, json_temp = send_esp_command(
            soba_id, {'CMD': 'GET_ROOM_TEMP', 'ID': termostat_id},
            timeout=TIMEOUT_ESP_SHORT
        )
        if json_temp and json_temp.get('status') == 'success':
            d = json_temp.get('data', {})
            room_data['currentTemp'] = d.get('room_temperature')
            room_data['setpoint'] = d.get('setpoint_temperature')
            mode = d.get('thermostat_control_mode', 0)
            room_data['isThermostatOn'] = (mode in [1, 2])
            room_data['thermostat_mode'] = {0: 'OFF', 1: 'HEATING', 2: 'COOLING'}.get(mode, 'OFF')

    # 2. GET_PINS za svaki kontroler
    for ctrl_id, pin_map in controller_to_pins.items():
        _, _, json_pins = send_esp_command(
            soba_id, {'CMD': 'GET_PINS', 'ID': ctrl_id},
            timeout=TIMEOUT_ESP_SHORT
        )
        if json_pins and json_pins.get('status') == 'success':
            pins_str = json_pins.get('data', {}).get('pin_states', '')
            for pin_str, dev_name in pin_map.items():
                idx = int(pin_str) - 1
                if 0 <= idx < len(pins_str):
                    room_data['lights'][dev_name] = (pins_str[idx] == '1')

    # 3. GET_ROOM_STATUS (card stacker)
    if scene_ctrl_id:
        _, _, json_room = send_esp_command(
            soba_id, {'CMD': 'GET_ROOM_STATUS', 'ID': scene_ctrl_id},
            timeout=TIMEOUT_ESP_SHORT
        )
        if json_room and json_room.get('status') == 'success':
            card = json_room.get('data', {}).get('card_inserted', False)
            room_data['card_inserted'] = card
            room_data['occupied'] = card

    # 4. GET_GUEST_IN_TEMP
    if scene_ctrl_id:
        _, _, json_in = send_esp_command(
            soba_id, {'CMD': 'GET_GUEST_IN_TEMP', 'ID': scene_ctrl_id},
            timeout=TIMEOUT_ESP_SHORT
        )
        if json_in and json_in.get('status') == 'success':
            room_data['guest_in_temp'] = json_in.get('data', {}).get('guest_in_temperature')

    # 5. GET_GUEST_OUT_TEMP
    if scene_ctrl_id:
        _, _, json_out = send_esp_command(
            soba_id, {'CMD': 'GET_GUEST_OUT_TEMP', 'ID': scene_ctrl_id},
            timeout=TIMEOUT_ESP_SHORT
        )
        if json_out and json_out.get('status') == 'success':
            room_data['guest_out_temp'] = json_out.get('data', {}).get('guest_out_temperature')

    # 6. GET_STATUS (SOS + parse za notifikaciju)
    _, _, json_status = send_esp_command(
        soba_id, {'CMD': 'GET_STATUS'}, timeout=TIMEOUT_ESP_SHORT
    )
    if json_status and json_status.get('status') == 'success':
        sos = json_status.get('data', {}).get('sos', {})
        room_data['sos_active'] = sos.get('active', False)
        room_data['sos_timestamp'] = sos.get('timestamp')
        parse_get_status_from_json(json_status, room_id=soba_id)

    # Upiši u background cache
    with _bg_room_cache_lock:
        _bg_room_cache[soba_id] = room_data

    logging.info(f"BG_CACHE: {soba_id} osvježen (temp={room_data['currentTemp']}, lights={len(room_data['lights'])}, card={room_data['card_inserted']})")

def background_resolver_task():
    """Pozadinski worker: osvježava cache za online sobe + mDNS resolve za offline sobe.
    Zamjenjuje stari resolver koji je samo slao GET_STATUS — sada radi puni cache refresh."""
    logging.info("Pokrenut pozadinski resolver + cache worker.")
    time.sleep(BG_CACHE_START_DELAY_SECONDS)  # Čekaj initial_map

    while True:
        try:
            cycle_start = time.time()
            logging.info("BG_CACHE: Pokrećem ciklus osvježavanja...")
            with config_lock:
                soba_ids = list(CONFIG.get('sobe', {}).keys())
            if not soba_ids:
                logging.info("BG_CACHE: Nema soba u config.json.")
                time.sleep(BG_CACHE_INTERVAL_SECONDS)
                continue

            for soba_id in soba_ids:
                with config_lock:
                    has_cached_ip = bool(CONFIG.get('sobe', {}).get(soba_id, {}).get('cached_ip'))

                if has_cached_ip:
                    # Soba online: puni cache refresh (temp + lights + card + SOS)
                    try:
                        _refresh_room_cache(soba_id)
                    except Exception as e:
                        logging.warning(f"BG_CACHE: Greška za {soba_id}: {e}")
                else:
                    # Soba nema IP: pokreni mDNS resolve
                    safe_start_resolve(soba_id)

                time.sleep(BG_CACHE_ROOM_DELAY_SECONDS)

            cycle_duration = time.time() - cycle_start
            logging.info(f"BG_CACHE: Ciklus završen za {cycle_duration:.1f}s.")

            # Čekaj ostatak intervala (ako je ciklus trajao kraće od intervala)
            wait_time = max(1, BG_CACHE_INTERVAL_SECONDS - cycle_duration)
            time.sleep(wait_time)

        except Exception as e:
            logging.error(f"Greška u background resolver/cache: {e}")
            time.sleep(BG_CACHE_INTERVAL_SECONDS)
...
def api_windows_get_rooms():
    ...
    Sve sobe se fetchuju paralelno (ThreadPoolExecutor) — ukupno čekanje =
    max(jedna soba) umjesto sum(sve sobe).
    """
    with config_lock:
        rooms_config = [
            (soba_id, dict(soba_config))
            for soba_id, soba_config in CONFIG.get('sobe', {}).items()
            if soba_config.get('uredjaji')
        ]

    def fetch_room(soba_id, soba_config):
        room_info = {
            "room_number": soba_id,
            ...
            "active_pin": "",
            "guest_pin": "",
            "pin_expiry": soba_config.get('pin_expiry'),
            "guest_in_temp": None,
            "guest_out_temp": None,
            "thermostat_mode": "OFF",
            "thermostat_on": False,
        ...
        if soba_config.get('cached_ip'):
            try:
                # GET_PASSWORD — UVIJEK LIVE (EEPROM = single source of truth)
                ...
                if json_pin is None:
                    raise ConnectionError("Soba ne odgovara na GET_PASSWORD")
                ...
                # OSTALO — čitaj iz background cache-a ako postoji
                with _bg_room_cache_lock:
                    bg = _bg_room_cache.get(soba_id)

                if bg and (time.time() - bg.get('last_update', 0)) < BG_CACHE_STALE_SECONDS:
                    # Popuni iz keša
                    ct = bg.get('currentTemp')
                    room_info['current_temp'] = int(ct) if ct is not None and ct == int(ct) else ct
                    sp = bg.get('setpoint')
                    room_info['setpoint_temp'] = int(sp) if sp is not None and sp == int(sp) else sp
                    room_info['target_temp'] = room_info['setpoint_temp']
                    room_info['card_inserted'] = bg.get('card_inserted', False)
                    room_info['occupied'] = bg.get('occupied', False)
                    room_info['thermostat_on'] = bg.get('isThermostatOn', False)
                    room_info['thermostat_mode'] = bg.get('thermostat_mode', 'OFF')
                    room_info['sos_active'] = bg.get('sos_active', False)
                    room_info['sos_timestamp'] = bg.get('sos_timestamp')
                    room_info['guest_in_temp'] = bg.get('guest_in_temp')
                    room_info['guest_out_temp'] = bg.get('guest_out_temp')
                else:
                    # BG cache prazan/zastareo — live fallback (stari kod)
                    ...
                    _, _, json_temp = send_esp_command(soba_id, {
                        'CMD': 'GET_ROOM_TEMP', 'ID': thermostat_id
                    }, timeout=TIMEOUT_ESP_SHORT)
                    if json_temp and json_temp.get('status') == 'success':
                    ...
                    _, _, json_room = send_esp_command(soba_id, {
                        'CMD': 'GET_ROOM_STATUS', 'ID': scene_controller_id
                    }, timeout=TIMEOUT_ESP_SHORT)
                    if json_room and json_room.get('status') == 'success':
                    ...
                    _, _, json_guest_in = send_esp_command(soba_id, {
                        'CMD': 'GET_GUEST_IN_TEMP', 'ID': scene_controller_id
                    }, timeout=TIMEOUT_ESP_SHORT)
                    if json_guest_in and json_guest_in.get('status') == 'success':
                        room_info['guest_in_temp'] = json_guest_in.get('data', {}).get('guest_in_temperature')

                    _, _, json_guest_out = send_esp_command(soba_id, {
                        'CMD': 'GET_GUEST_OUT_TEMP', 'ID': scene_controller_id
                    }, timeout=TIMEOUT_ESP_SHORT)
                    if json_guest_out and json_guest_out.get('status') == 'success':
                        room_info['guest_out_temp'] = json_guest_out.get('data', {}).get('guest_out_temperature')
                ...
                # Uspješan fetch — sačuvaj u _room_status_cache kao zadnje poznato dobro stanje
                with _room_status_cache_lock:
                    _room_status_cache[soba_id] = dict(room_info)

            except Exception as e:
                logging.error(f"Greška pri čitanju statusa sobe {soba_id}: {e}")
                with _room_status_cache_lock:
                    cached = _room_status_cache.get(soba_id)
                if cached:
                    logging.warning(f"Soba {soba_id}: fetch failo, vraćam cached status.")
                    return dict(cached)

        return room_info

    # Paralelno fetchuj sve sobe — max 3 istovremena zahtjeva da ne overloadamo WiFi AP
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        futures = {executor.submit(fetch_room, soba_id, soba_config): soba_id
                   for soba_id, soba_config in rooms_config}
        rooms_data = [f.result() for f in concurrent.futures.as_completed(futures)]
...
@app.route('/api/rooms/rediscover', methods=['POST'])
@require_api_key
def api_rooms_rediscover():
    """Pokreće odmah mDNS rediscovery za sve offline sobe.

    Koristi se kada je bila slaba konekcija i neke sobe su izgubile cached_ip.
    Umjesto čekanja do sljedećeg background resolver ciklusa (60s), ovaj endpoint
    odmah trigerira safe_start_resolve() za svaku offline sobu.

    Vraća odmah (ne čeka na završetak resolucije).
    Klijent treba pričekati ~3-4s pa refreshati GET /api/rooms.
    """
    triggered = []
    already_in_progress = []
    ...
    for soba_id, soba_config in rooms_config:
        if not soba_config.get('uredjaji'):
            continue
        if not soba_config.get('cached_ip'):
            with _resolving_registry_lock:
                already = soba_id in _resolving_in_progress
            if already:
                already_in_progress.append(soba_id)
            else:
                safe_start_resolve(soba_id)
                triggered.append(soba_id)

    logging.info(
        f"Rediscovery pokrenut: {len(triggered)} soba ({triggered}), "
        f"{len(already_in_progress)} već u toku."
    )
    return jsonify({
        "status": "success",
        "data": {
            "triggered": len(triggered),
            "already_resolving": len(already_in_progress),
            "rooms": triggered
        },
        "message": f"Rediscovery pokrenut za {len(triggered)} offline soba."
    })
...
def api_windows_set_pin(room_number):
    ...
    try:
        ...
        # SAFEGUARD 2: Invalidiraj cache za ovu sobu (check-in)
        with _bg_room_cache_lock:
            _bg_room_cache.pop(room_number, None)
        with _room_status_cache_lock:
            _room_status_cache.pop(room_number, None)
...
def api_windows_delete_pin(room_number):
    ...
    try:
        ...
        # SAFEGUARD 2: Invalidiraj cache za ovu sobu (check-out)
        with _bg_room_cache_lock:
            _bg_room_cache.pop(room_number, None)
        with _room_status_cache_lock:
            _room_status_cache.pop(room_number, None)
        ...
        try:
            ...
            if pending_rooms:
                ...
                for room_id in pending_rooms:
                    if _sync_staff_pins_to_room(room_id, maid_pin, manager_pin, service_pin):
                        any_synced = True
                if any_synced:
                    save_config()
            else:
    ...
    while True:
        try:
            ...
            for room_id, room_data in rooms.items():
                ...
                while True:
                    try:
                        ...
                        if app_response.status_code == 200:
                            ...
                            time.sleep(LOG_TRANSFER_DELETE_DELAY_SECONDS)
                        else:
                ...
                time.sleep(LOG_TRANSFER_ROOM_DELAY_SECONDS)
            
            logging.info(f"[LogTransfer] Cycle complete: {total_logs} logs transferred")
            time.sleep(LOG_TRANSFER_INTERVAL_SECONDS)
...
@require_api_key
def api_get_logs():
    """Logovi se čuvaju lokalno u APP-ovoj SQLite bazi (ne na serveru).
    Endpoint postoji da spriječi 404 kada APP pozove GET /api/logs."""
    return jsonify({'status': 'success', 'data': {'logs': [], 'total': 0}})


@app.route('/api/log_transfer/register_app', methods=['POST'])
def register_app():
    """APP se registruje kada se pokrene. URL se snima u config.json
    kako bi bio dostupan i nakon restarta servera."""
    global APP_ENDPOINT_URL
    ...
    if app_url:
        ...
        # Persist u config.json — server restart neće izgubiti registraciju
        with config_lock:
            CONFIG['app_endpoint_url'] = APP_ENDPOINT_URL
        save_config()
        return jsonify({'status': 'success'})

    return jsonify({'status': 'error', 'message': 'app_url missing'}), 400
...
if __name__ == '__main__':
    ...
    def initial_map():
        for soba_id in initial_soba_ids:
            resolve_and_cache_ip(soba_id)
            time.sleep(INITIAL_MAP_ROOM_DELAY_SECONDS)
